...
A speed and correctness release for the first thing you ask of a page — and, on Apple Silicon, the release that makes MLX models work again. **MLX models could not start at all on Apple Silicon in 0.4.2 through 0.5.2** — including the setup wizard's own pre-selected recommendation for your Mac. It failed with *“the bundled MLX Python runtime is missing”*, and the message told you to reinstall, which could never have helped: every installer since 0.4.2 had the same layout. Nothing was actually missing — the app checked for the Python runtime under one filename while the build had been producing another, so it refused to launch a sidecar that was present and working. **If you tried an MLX model in the last three weeks and concluded something was wrong with your setup: it was not.** The llama.cpp backend, which is the default and the only one Windows uses, was never affected. Separately, **MLX generations now receive their instructions** — they were being sent with no system prompt at all, so the curated guidance every model in the catalogue was measured against never reached them. **A form page no longer costs two generations**: the quality check flagged the back link the app itself adds as a dead button, so the score could never pass and a corrective model pass ran on almost every page. A page that needs no correction now arrives in **about half the time** — roughly 30 seconds against 52–58 when one runs. The corrective pass is not gone, but a cause that could never be satisfied has been removed, and **it can no longer replace a finished page with a near-empty one**. Also: **accessibility checks run once per change and again after you switch page** (a manual check during a pending one ran two scans; switching page produced no check at all), **typing no longer waits for a check that starts mid-sentence**, and an NHS.UK class that does not exist is no longer offered to the model. **Available now for macOS**; the Windows installer follows shortly and will be attached to the same release. Any 0.2.x, 0.3.x, 0.4.x or 0.5.x install can move to 0.6.0 in place via **Settings → Updates**. See the [release notes](https://github.com/digitalcourtney87/prompt-to-page/releases/latest) for the full list.
