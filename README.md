...
A generation-quality release for the two newest design-system packs, plus a dependency security update — no new features. **NSW pack: JavaScript-enhanced components now generate usable markup** — where the pack's bundled guidance only listed class names, models invented `nsw-*` class names that match nothing in the design system's CSS; the guidance now carries the real markup for tabs, accordion, side navigation and friends, so generated NSW pages render with the design system's actual styles. **Bootstrap Italia pack: confirmation and journey pages now use patterns that work in a static prototype** — a visible success alert instead of a JavaScript toast that never fires, "Start now" journey links, check-your-answers rows, and English example content instead of Italian button labels. **DOMPurify is updated to 3.4.14** (GHSA-55q2-fjhq-7xh7 — the app was not exposed, but DOMPurify is the seam that scrubs model-generated markup, so it is kept current). The 0.4.4 Windows installer is not attached yet; Windows testers can install 0.4.3 and the app updates itself in place once it lands. On macOS, any 0.2.x, 0.3.x or 0.4.x install can move to 0.4.4 in place via **Settings → Updates**. See the [release notes](https://github.com/digitalcourtney87/prompt-to-page/releases/latest) for the full list.
