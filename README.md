...
A journeys release. A prototype can now **branch on an answer**: on a question Page with a radio, single-select or checkbox group, **Route by answer** in the Journey tab lets you say which answer goes to which Page, with an **Otherwise go to** fallback; the preview, a LAN share, the testable ZIP and a Prototype Kit hand-off all follow the same decision. A Journey is also more honest about what it knows: renaming a Page heals the links and form actions that pointed at it, a question with no destination is reported as a decision still to make rather than quietly pointed at the next page, and a form with no usable destination is raised as a quality warning. **Each Page now records which model and runtime produced or refined it**, and the assessment evidence describes those contributions per Page instead of naming one model for the whole prototype. Two Prototype Kit hand-off faults are fixed — exported forms posted one Page too far so a routing decision was never reached, and the check answers view did not render — and a check answers row backed by day, month and year inputs now shows the date you typed instead of the sample in the generated page. Also: a failed library save is reported with **Retry saving**; a late model result can no longer land in a different project or page; a refinement that would drop form fields asks first; Stop, restart and retry for the local AI are more predictable; and preview canvas actions are keyboard-reachable. **macOS build available now; the Windows build follows shortly.** Any earlier install can move to 0.6.1 in place via **Settings → Updates** once its platform's build is published. See the [release notes](https://github.com/digitalcourtney87/prompt-to-page/releases/latest) for the full list.
## Requirements
