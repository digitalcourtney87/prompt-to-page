...
A page-editing release. **Add section is its own operation, separate from editing what already exists** — the preview toolbar gains **Add section** beside **Edit section**: choose a visible, keyboard-focusable **Add here** point inside the page's main content, describe the section, and generate once; or type an additive request and let a named position (*at the end*, *before the form*) place it when it matches exactly one boundary, otherwise choose a point once. Existing content is never sent to the model and stays byte-for-byte unchanged unless the new markup passes sanitisation, the pack's class allowlist and structure checks; one request makes one Version history entry, **Undo** reverses it, and Esc while placing keeps the request in the chat with **Choose placement**. **Second prompts resolve their intent before their target** — an addition, a change to an existing section, or a confirmed whole-page regeneration — with clarifying questions (Add a new section / Change an existing section; Add another / Change existing; Create an equivalent / Cancel) instead of guesses. Abstract gains Resume and Schedule moments plus new variants. Fixes: Esc on a confirmation sheet no longer chooses the wrong button on macOS (Esc on "Delete project?" used to delete the project); a Settings save with a pinned port no longer leaves the app without a local model; GOV.UK Frontend behaviour (accordions, tabs, character counts, error-summary focus) now runs in the preview, the LAN share and the exported prototype, which it never had; saving Settings no longer restarts the local AI sidecar twice; the testable prototype's check-your-answers rows fill in correctly on every pack; LAN share initialises each page with its own design system in a mixed-pack project; Scottish Government check-answers rows are recognised by Exit resolution; code-view edits no longer disturb the Validation panel; first-run setup no longer shows two Start model download buttons; the Beta terms summary in Settings says usage statistics are off by default. The macOS build is available now; the Windows 0.5.1 installer follows once its sitting is done (0.5.0 Windows installs move to 0.5.1 in place when it lands). Any 0.2.x, 0.3.x, 0.4.x or 0.5.0 macOS install can move to 0.5.1 in place via **Settings → Updates**. See the [release notes](https://github.com/digitalcourtney87/prompt-to-page/releases/latest) for the full list.
